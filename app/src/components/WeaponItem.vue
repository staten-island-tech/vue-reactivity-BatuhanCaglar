<template>
  <div>
    ...
    <div v-if="!selectedWeapon" class="carousel w-full">
      <div v-for="(weapon, index) in weapons" :key="weapon.name" :id="`item${index + 1}`" class="carousel-item w-full">
        <img :src="weapon.image" :alt="weapon.name" class="w-full cursor-pointer" @click="selectWeapon(weapon)" />
      </div>
    </div>

    <div v-if="!selectedWeapon" class="flex w-full justify-center gap-2 py-2">
      <a v-for="(weapon, index) in weapons" :key="weapon.name" :href="`#item${index + 1}`" class="btn btn-xs">
        {{ index + 1 }}
      </a>
    </div>

    <!-- Selected Weapon & Enchantment Selection -->
    <div v-if="selectedWeapon && !isFinished" class="selected-weapon">
      ...
      <div class="selected-enchantments">
        <h3>Current Enchantments:</h3>
        <span v-if="selectedEnchantments.length === 0">None</span>
        <span v-for="enchantment in selectedEnchantments" :key="enchantment" class="badge">{{ enchantment }}</span>
      </div>

      <!-- Enchantment Selection -->
      <div v-if="currentStage <= 3 && availableEnchantments.length > 0" class="enchantments">
      ...
      <!-- Next Button -->
      <div v-if="selectedEnchantments.length === currentStage && currentStage < 3 && availableEnchantments.length > 0" class="controls">
        <button @click="nextStage" class="btn mt-4">Next</button>
      </div>

      <!-- Finish Selection -->
      <div v-if="selectedEnchantments.length > 0" class="controls">
        <button @click="finishSelection" class="btn mt-4">Finish</button>
      </div>
    ...
    <div v-if="isFinished" class="final-selection">
...
const currentStage = ref(1) // Track the current enchantment stage (1, 2, or 3)
const isFinished = ref(false) // Track when user completes selection

const availableEnchantments = computed(() => {
  return allEnchantments.filter(e =>
...
.selected-weapon, .final-selection {
  text-align: center;
  margin-top: 20px;
}
.selected-enchantments, .final-selection {
  margin-top: 10px;
  font-size: 1.1em;
}
.badge {
